...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: (process.env.SMTP_PASS || '').replace(/\s+/g, ''), // Fix: Remove spaces from app password
  },
...
// Verify connection on startup (this helps debug logs immediately)
transporter.verify((error, success) => {
  if (error) {
    console.error('📧 SMTP Verification Failed:', error.message);
    if (error.code === 'ETIMEDOUT') {
      console.warn('⚠️ SMTP Timeout: Check if port 465/587 is open and SMTP_HOST is correct.');
    }
  } else {
