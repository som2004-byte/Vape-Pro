...
const sendEmail = async ({ to, subject, html, bcc }) => {
  try {
    ...
    const { data, error } = await resend.emails.send({
      ...
      bcc: bcc ? (typeof bcc === 'string' ? [bcc] : bcc) : undefined,
      subject: subject,
      html: html,
    });
...
const sendOtpEmail = async (to, otp) => {
  return sendEmail({
    to,
    subject: 'Verify your VapeSmart login',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; padding: 20px; border: 1px solid #eee;">
        <h2 style="color: #333;">Email Verification</h2>
        <p>Your verification code for VapeSmart is:</p>
...
const sendOrderConfirmationEmail = async (toEmail, order) => {
  ...
  return sendEmail({
    to: toEmail,
    bcc: adminEmail || undefined, // BCC Admin if exists
...
const sendAdminNewOrderEmail = async (order, customer = null) => {
  ...
  if (!targetEmail) {
    console.warn('[EMAIL] ⚠️ ADMIN_EMAIL is missing. Cannot send admin alert.');
    return false;
  }
